...
<body>
    ...
    <section class="products">
        ...
        <div class="box-container">
            ...
            <?php
      $grand_total = 0;
      $select_wishlist = $conn->prepare("SELECT wishlist.id, wishlist.user_id, wishlist.pid, products.p_name, products.price,products.P_quantity, products.image_01 FROM wishlist INNER JOIN products ON wishlist.pid = products.id WHERE wishlist.user_id = ? ");
   ...
   ?>
            <form action="" method="post" class="box">
                ...
                <input type="hidden" name="name" value="<?= $fetch_wishlist['p_name']; ?>">
                ...
                <div class="name"><?= $fetch_wishlist['p_name']; ?></div>
                <div class="flex">
                    <div class="price">Rs. <?= $fetch_wishlist['price']; ?>/-</div>
                    <input type="number" name="qty" class="qty" min="1" max="<?=$fetch_wishlist['P_quantity'] ?>"
                        onkeypress="if(this.value.length == 2) return false;" value="1">
                </div>
                <?php
                if($fetch_wishlist['P_quantity']>=10)
                {
                    echo '<span class="greenText">Stock</span>';
                    echo '<input type="submit" value="Add to cart" class="btn" name="add_to_cart">';
                    }
                    elseif($fetch_wishlist['P_quantity']<10 && $fetch_wishlist['P_quantity']>=1){
                        echo '<span class="warningText">' . $fetch_wishlist['P_quantity'] . ' items left in stock</span>';
                        echo '<input type="submit" value="Add to cart" class="btn warning-btn"  name="add_to_cart">';
                    }
                    else
                    {
                        echo '<span class="redText">' . $fetch_wishlist['P_quantity'] . ' items left in stock</span>';
